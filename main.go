...
var (
	...
	// small buffer pool to avoid allocations for copy loops and UDP buffers
	bufPool = sync.Pool{
		New: func() interface{} {
...
func main() {
	...
	// be explicit about CPU usage - no behavioral change on modern Go but clarifies intent
...
func handleRelayConnection(conn net.Conn) {
	...
	if ruleLen == 0 {
		...
		for _, rule := range udpRules {
			...
			// moderate OS buffers to balance throughput vs buffer-bloat risk
			_ = udpConn.SetReadBuffer(256 * 1024)  // 256KB
			_ = udpConn.SetWriteBuffer(256 * 1024) // 256KB
...
func startTCPForwarderWithListener(session *smux.Session, rule ForwardRule, listener net.Listener) {
	...
	for {
		...
		go func(c net.Conn) {
			...
			// Bidirectional copy using pooled buffers for fewer allocations
			...
			go func() {
				...
				// put buffer back
				bufPool.Put(bufPtr)
				// close the other side to release resources faster
				_ = stream.Close()
			}()
	...
	for {
		...
		if !exists {
			...
			go func(s *smux.Stream, session *UDPSession, sessKey string) {
				...
				// return buffer
				bufPool.Put(respBufPtr)
			}(stream, sess, keyCopy)
		}

		// send to stream with length prefix while holding per-session lock only
		sess.mu.Lock()
...
func (rm *RelayManager) maintainConnection(relay *RelayConnection) {
	for {
		...
		// Connection lost; clear pointers under lock to avoid races
		relay.mu.Lock()
...
func handleTCPStream(stream *smux.Stream, targetPort string) {
	...
	target, err := dialer.Dial("tcp", "127.0.0.1:"+targetPort)
	if err != nil {
	...
	// Stream -> target (from relay)
	go func() {
		defer wg.Done()
		bufPtr := bufPool.Get().(*[]byte)
		buf := *bufPtr
		_, _ = io.CopyBuffer(target, stream, buf)
		bufPool.Put(bufPtr)
		_ = target.Close()
	}()

	// Target -> stream (to relay)
	go func() {
		defer wg.Done()
		bufPtr := bufPool.Get().(*[]byte)
		buf := *bufPtr
		_, _ = io.CopyBuffer(stream, target, buf)
		bufPool.Put(bufPtr)
		_ = stream.Close()
	}()
