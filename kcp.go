...
func runRelayTCP() {
	...
	log.Printf("Relay server listening on TCP :%s", *port)
...
func runRelayKCP() {
	...
	log.Printf("Relay server listening on KCP (UDP) :%s [mode=%s ds=%d ps=%d mtu=%d]",
		*port, *kcpMode, *kcpDS, *kcpPS, *kcpMTU)
...
func runVPN() {
	...
	log.Printf("Strategy: %s, Transport: %s", *strategy, *transport)
...
func (rm *RelayManager) maintainConnection(relay *RelayConnection) {
	for {
		...
		// For KCP we set a deadline on the underlying connection
		if kcpSess, ok := conn.(*kcp.UDPSession); ok {
			kcpSess.SetDeadline(time.Now().Add(10 * time.Second))
		} else {
